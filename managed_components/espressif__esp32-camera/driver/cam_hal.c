...
static esp_err_t cam_dma_config(const camera_config_t *config)
{
    ...
    for (int x = 0; x < cam_obj->frame_cnt; x++) {
        ...
        cam_obj->frames[x].fb_alloc = NULL;
        cam_obj->frames[x].fb_offset = 0;
        cam_obj->frames[x].en = 0;
        ESP_LOGI(TAG, "Allocating %d Byte frame buffer in %s", alloc_size, _caps & MALLOC_CAP_SPIRAM ? "PSRAM" : "OnBoard RAM");
        cam_obj->frames[x].fb_alloc = heap_caps_malloc(alloc_size, _caps);
        CAM_CHECK(cam_obj->frames[x].fb_alloc != NULL, "frame buffer malloc failed", ESP_FAIL);
        cam_obj->frames[x].fb.buf = (uint8_t *)cam_obj->frames[x].fb_alloc;
        if (cam_obj->psram_mode) {
            uintptr_t misalign = ((uintptr_t)cam_obj->frames[x].fb.buf & (dma_align - 1));
            cam_obj->frames[x].fb_offset = misalign ? (dma_align - misalign) : 0;
...
esp_err_t cam_deinit(void)
{
    ...
    if (cam_obj->frames) {
        for (int x = 0; x < cam_obj->frame_cnt; x++) {
            if (cam_obj->frames[x].fb_alloc) {
                heap_caps_free(cam_obj->frames[x].fb_alloc);
            }
