...
static void user_wifi_disconnect_request_handler(void* arg, esp_event_base_t event_base,
...
void wifi_init() 
{
    ...
    ESP_ERROR_CHECK(esp_event_handler_register_with(ui_event_loop_handle, APP_EVENT, APP_WIFI_DISCONNECT_REQUEST, user_wifi_disconnect_request_handler, NULL));
       ...
       switch (event_id) 
       {
        ...
        case WIFI_EVENT_SCAN_DONE:
            ...
            if (current_wifi_state == WIFI_STATE_BUSY) 
            {
                ...
                if (ap_total_count > 0) {
                    ap_records = (wifi_ap_record_t *)heap_caps_malloc(
                        sizeof(wifi_ap_record_t) * ap_total_count,
                ...
                if (ap_total_count > 0) {
                    ESP_ERROR_CHECK(esp_wifi_scan_get_ap_records(&ap_total_count,
                                                                 ap_records));
                }

                wifi_scan_done_data_t data = {.ap_count = ap_total_count, .ap_records = ap_records};
        ...
        case WIFI_EVENT_STA_DISCONNECTED:
            ...
            else
            {
                ...
                wifi_disconnect_event_data_t disconnect_data = {
                    .reason = disconnect_event == NULL ? WIFI_REASON_UNSPECIFIED
                                                       : disconnect_event->reason,
                    .requested_by_user = user_disconnect_requested ? 1 : 0,
                };

                esp_event_post_to(ui_event_loop_handle, APP_EVENT, APP_WIFI_DISCONNECTED,
                                  &disconnect_data, sizeof(disconnect_data),
                                  portMAX_DELAY);
                                                 ...
                                                 int32_t event_id, void* event_data)
{
    if (xSemaphoreTake(wifi_mutex, pdMS_TO_TICKS(5000)) != pdTRUE)
    {
        ESP_LOGE(TAG, "Mutex take timeout in DISCONNECT handler");
        return;
    }
    ...
    if (current_wifi_state != WIFI_STATE_CONNECTED &&
        current_wifi_state != WIFI_STATE_GETIP)
    {
        ESP_LOGW(TAG, "WiFi not connected, ignoring disconnect request");
        xSemaphoreGive(wifi_mutex);
        return;
    }

    user_disconnect_requested = true;
    ESP_LOGI(TAG, "Disconnecting current WiFi connection");
