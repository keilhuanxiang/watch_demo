...
const char* event_to_string(audio_player_callback_event_t event) {
    ...
    case AUDIO_PLAYER_CALLBACK_EVENT_PLAYBACK_FAILED:
        return "AUDIO_PLAYER_CALLBACK_EVENT_PLAYBACK_FAILED";
...
static esp_err_t aplay_file(audio_instance_t *i, FILE *fp) {
    ...
    if(is_mp3(fp)) {
        ...
        i->mp3_data.continuous_error_count = 0;
        i->mp3_data.continuous_sync_miss_count = 0;
        skip_mp3_id3v2_tag(fp);
    }
#endif
    ...
    do {
        ...
        {
            if (i->output.frame_count == 0 ||
                i->output.fmt.sample_rate <= 0 ||
                i->output.fmt.bits_per_sample <= 0 ||
                i->output.fmt.channels <= 0) {
                LOGI_1("skipping invalid output: frames=%lu sr=%lu bps=%lu ch=%lu",
                       i->output.frame_count,
                       i->output.fmt.sample_rate,
                       i->output.fmt.bits_per_sample,
        ...
        } else if(decode_status == DECODE_STATUS_NO_DATA_CONTINUE)
        {
            ...
            if (decode_status == DECODE_STATUS_ERROR) {
                dispatch_callback(i, AUDIO_PLAYER_CALLBACK_EVENT_PLAYBACK_FAILED);
            }
...
#if defined(CONFIG_AUDIO_PLAYER_ENABLE_MP3)
    i->mp3_data.data_buf_size = MAINBUF_SIZE * 6;
